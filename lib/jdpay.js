...
Jdpay.prototype.route = function(app) {
    var self = this;
    app.post(this.jdpay_config.jdpay_import_return_url, function(req, res) {
        self.jdpay_import_return(req, res)
    });
    app.post(this.jdpay_config.jdpay_import_notify_url, middleware.jdpay.Decode, function(req, res) {
...
Jdpay.prototype.refund = function(data, res) {
    ...
    var postData = jdpaySubmit.buildRequestForm(parameter),
...
Jdpay.prototype.jdpay_import_notify = function(req, res) {
    ...
    //验证消息是否是京东发出的合法消息
    jdpayNotify.verifyNotify(result, function(verify_result) {
        if (verify_result) { //验证成功
            var info = _.pick(_POST, infoList);
            if (_POST.is_success == 'Y') {
                //trade_class 交易类别，用于区分支付和退款结果通知 SALE:消费/支付,REFD退款
                self.emit('jdpay_import_trade_success' + _POST.trade_class, info);
            } else {
                self.emit('jdpay_import_trade_fail' + _POST.trade_class, _.merge(info, {
 ...
 */
Jdpay.prototype.query_order = function(data, res) {
    //建立请求
    var jdpaySubmit = new JdpaySubmit(this.jdpay_config),
        jdpayNotify = new JdpayNotify(this.jdpay_config);
